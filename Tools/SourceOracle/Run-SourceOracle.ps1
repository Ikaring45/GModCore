...
if (-not (Test-Path -LiteralPath $addonSource -PathType Container)) {
    throw "Oracle addon source not found: $addonSource"
}
...
try {
    ...
    $mount.WriteGeneratedFile('lua\garryspad_oracle\run_token.txt', $runID)
    ...
    $arguments = @(
        ...
        '-insecure',
        '+map', 'gm_flatgrass',
        '+garryspad_source_oracle_run', $runID
    )
    ...
    if ($null -eq $validatedResult) {
        ...
        throw "Garry's Mod oracle produced no authenticated result ($exitDescription$validationDescription)"
    }

    Copy-Item -LiteralPath $gameResult -Destination $savedResult
    Read-SourceOracleResult -Path $savedResult -RunID $runID -Realm 'SERVER' | Out-Null
    ...
    if ($null -ne $ownedJob) {
        try {
            $ownedJobStopped = $ownedJob.TerminateAndWait(10000, [uint32]0xE0450002)
