...
function PANEL:Init()
	...
	-- Detect whether the HTML engine is even there
...
function OnMenuFailedToLoad()
	...
	lbl:SetText( "Looks like the main menu failed to load.\n\nPlease exit the game and try running GModPatchTool again.\n\nIf that doesn't work, review the GModPatchTool Guide/FAQ. If that doesn't work, join the GModPatchTool Discord for help. The button to exit the game is also below." )

	local btn_guide = frame:Add( "DButton" )
	btn_guide:Dock( TOP )
	btn_guide:DockMargin( 0, 0, 0, 5 )
	btn_guide:SetText( "Open the GModPatchTool Guide / FAQ" )
	btn_guide.DoClick = function() gui.OpenURL("https://solsticegamestudios.com/fixmedia/") end

	local btn_contact = frame:Add( "DButton" )
	btn_contact:Dock( TOP )
	btn_contact:DockMargin( 0, 0, 0, 5 )
	btn_contact:SetText( "Join the GModPatchTool Discord" )
	btn_contact.DoClick = function() gui.OpenURL("https://solsticegamestudios.com/discord/") end
